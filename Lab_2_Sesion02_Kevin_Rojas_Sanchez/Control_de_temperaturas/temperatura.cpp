...
double* crearRegistro(int& cantidadDias)
{
	...
	{ 
		std::cout << "La cantidad de dias debe ser mayor a cero. Intente nuevamente: ";
		std::cin >> cantidadDias;
	}
...
{
	std::cout << "Ingrese las temperaturas para " << cantidad << " dias:" << std::endl;
...
void mostrarResultados(const double* registro, int cantidad)
{
	...
	std::cout << "Promedio de temperaturas: " << calcularPromedio(registro, cantidad) << "°C" << std::endl;
